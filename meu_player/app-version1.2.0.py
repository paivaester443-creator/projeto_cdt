...
def callback():
    ...
    # Busca faixas no acervo global do Spotify
    termo_busca = f"{humor} {clima['tag']}"
    resultados = sp.search(q=termo_busca, type='track', limit=15)
    tracks_uris = [item['uri'] for item in resultados['tracks']['items']]
    
    # Cria a playlist na conta do usuário
    nome_playlist = f"Vibe {humor} | Clima {clima['tag']} ({cidade})"
    playlist = sp.user_playlist_create(
        user=user_id, 
        name=nome_playlist, 
        public=True,
        description=f"Playlist criada via automação para {cidade} ({clima['desc']}, {clima['temp']}°C)."
    )
    ...
    <div style="font-family: sans-serif; text-align: center; padding: 60px; background: #121212; color: #fff; height: 100vh;">
        <h1 style="color: #1db954;">🎉 Playlist criada com sucesso no seu Spotify!</h1>
        <p style="font-size: 1.2rem; margin-top: 10px;"><strong>Nome:</strong> {nome_playlist}</p>
        ...
        <br><br>
        <a href="/" style="color: #aaa; text-decoration: none;">← Criar outra playlist</a>
    </div>
    """
